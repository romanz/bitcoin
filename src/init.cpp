...
void Shutdown(NodeContext& node)
{
    ...
    if (g_offsets_index) g_offsets_index.reset();
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-blockfilterindex=<type>",
                 ...
                 ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-offsetsindex", strprintf("Maintain a per-block transactions' offsets' index (default: %u)", DEFAULT_OFFSETSINDEX), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.GetBoolArg("-offsetsindex", DEFAULT_OFFSETSINDEX)) {
        g_offsets_index = std::make_unique<OffsetsIndex>(interfaces::MakeChain(node), /*cache_size=*/0, false, do_reindex);
        node.indexes.emplace_back(g_offsets_index.get());
    }
