...
struct DBKey {
    ...
    SERIALIZE_METHODS(DBKey, obj) {
        ...
        if (prefix != DB_BLOCK_HASH) {
            throw std::ios_base::failure("Invalid format for offset index DB hash key");
        }
...
std::unique_ptr<OffsetsIndex> g_offsets_index;

OffsetsIndex::OffsetsIndex(std::unique_ptr<interfaces::Chain> chain,
                         size_t n_cache_size, bool f_memory, bool f_wipe)
    : BaseIndex(std::move(chain), "offsets"), m_block_header_size(::GetSerializeSize(CBlockHeader{}))
{
    fs::path path = gArgs.GetDataDirNet() / "indexes" / "offsets";
    fs::create_directories(path);

    m_db = std::make_unique<BaseIndex::DB>(path / "db", n_cache_size, f_memory, f_wipe);
}

bool OffsetsIndex::CustomAppend(const interfaces::BlockInfo& block)
{
    assert(block.data);
    assert(block.file_number >= 0);

    const uint32_t nTx = block.data->vtx.size();
    uint32_t nTxOffset = m_block_header_size + GetSizeOfCompactSize(nTx);
...
bool OffsetsIndex::ReadRawTransaction(uint256 block_hash, size_t i, std::vector<uint8_t>& out) const
{
    ...
    if (value.offsets.empty()) {
        LogError("%s: OffsetsIndex entry for %s:%u must have >1 offsets\n", __func__, block_hash.ToString(), part);
        return false;
    }
    size_t nTx = value.offsets.size() - 1;
    if (i >= nTx) {
        LogError("%s: OffsetsIndex entry for %s:%u has %d transactions\n", __func__, block_hash.ToString(), part, nTx);
        return false;
    }
    ...
    try {
        ...
        LogError("%s: Read %d bytes from %s failed: %s for %s\n", __func__, tx_size, tx_pos.ToString(), e.what());
        return false;
    }
