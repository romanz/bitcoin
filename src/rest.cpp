...
static bool rest_tx_from_block(const std::any& context, HTTPRequest* req, const std::string& strURIPart)
{
    ...
    OffsetsIndex* offsets_index = g_offsets_index.get();
    if (!offsets_index) {
        return RESTERR(req, HTTP_BAD_REQUEST, "Offset index is not enabled");
    }

    if (!offsets_index->BlockUntilSyncedToCurrentChain()) {
        RESTERR(req, HTTP_SERVICE_UNAVAILABLE, "Offset index is still syncing");
    }

    std::vector<uint8_t> out;
    if (!offsets_index->ReadRawTransaction(*block_hash, index, out)) {
        return RESTERR(req, HTTP_NOT_FOUND, strprintf("Failed to read transaction #%d from block %s",
                                                      index, (*block_hash).ToString()));
    }
